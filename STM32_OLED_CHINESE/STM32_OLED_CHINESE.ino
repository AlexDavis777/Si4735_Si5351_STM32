...
#include <EEPROM.h>
#include <SI4735.h>
#include "DSEG7_Classic_Mini_Bold_14.h"
#include "Rotary.h"
...
// Buttons controllers
#define MODE_SWITCH PA9       // Switch MODE (Am/LSB/USB)
#define BANDWIDTH_BUTTON PA8  // Used to select the banddwith.
#define VOLUME_BUTTON PA7     // Volume Up
#define AVC_BUTTON PA6        // **** Use thi button to implement a new function
#define BAND_BUTTON PA5       // Next band
#define SOFTMUTE_BUTTON PA4   // **** Use thi button to implement a new function
#define AGC_BUTTON PA2       // Switch AGC ON/OF
#define STEP_BUTTON PA3      // Used to select the increment or decrement frequency step (see tabStep)
#define ENCODER_BUTTON PA15   // Used to select the enconder control (BFO or VFO) and SEEK function on AM and FM modes
#define RESET_PIN PB12
#define BEEPER PA11  //(Beeper)   
...
void setup() {
  ...
  delay(200);// give display time to initialise
  display.clearDisplay();
  display.setTextColor(WHITE);

  // Splash - Change orig for your introduction text or remove the splash code.
  display.setTextColor(BLACK,WHITE);
  display.setCursor(26, 5);
  display.print("SI473X/STM32");
  display.setTextColor(WHITE);
  display.setCursor(16, 20);
  display.print("Arduino Library");
  delay(300);
  display.setCursor(25, 35);
  display.print("STM32 Radio");
  display.display();
  delay(300);
  display.setCursor(12, 50);
  display.print("V3.0.8-By PU2CLR");
  display.display();
    // end Splash
    
  beeperOn = true;
  Beep(2, 4);//(no of beeps,duration)
  beeperOn = false;
  ...
  vfo.set_freq(3276800, SI5351_CLK1); //Start CLK1 (VFO) 3276800
  vfo.output_enable(SI5351_CLK0, 0); // turn off CLK0
  vfo.output_enable(SI5351_CLK2, 0); // turn off CLK2
  #endif
...
// Comment the line above and uncomment the three lines below if you are using external ref clock (active crystal or signal generator)
  ...
  si4735.setup(RESET_PIN, 0, MW_BAND_TYPE, SI473X_ANALOG_AUDIO, XOSCEN_RCLK);
  #endif
  delay(500);
...
void loadSSB() {
  ...
  si4735.setI2CFastModeCustom(400000);
  //si4735.queryLibraryId();  // Is it really necessary here? I will check it.
  //si4735.patchPowerUp();
  delay(50);
  si4735.loadPatch(ssb_patch_content, size_content, bandwidthSSB[bwIdxSSB].idx);
